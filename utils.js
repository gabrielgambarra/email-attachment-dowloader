...
const downloadAttachment = async (gmail) => {
    try {
        const { data } = await gmail.users.messages.list({ userId: 'me', q: 'has:attachment is:unread', maxResults: 10 });
        ...
        for (const { id } of data.messages) {
            ...
            for (const part of parts) {
                if (part.filename && part.body?.attachmentId) {
                    await download(gmail, 'me', id, part.body.attachmentId, part.filename);
                }
            }
        }
    } catch (error) {
